...
fn parse_md(product: &Product) -> Option<Event> {
    ...
    let watch_issuance_probability =
        if let Some(watch_issuance_probability) = regexes.probability.captures(&text) {
            Some(safe_result!(watch_issuance_probability[1].parse::<u16>()))
        } else {
            None
        };
